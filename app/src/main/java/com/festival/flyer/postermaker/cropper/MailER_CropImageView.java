...
package com.festival.flyer.postermaker.cropper;
...
public class MailER_CropImageView extends FrameLayout {
  ...
  private final MailER_CropOverlayView mCropOverlayView;
  ...
  private MailER_CropImageAnimation mAnimation;
  ...
  private WeakReference<MailER_BitmapLoadingWorkerTask> mBitmapLoadingWorkerTask;

  /** Task used to crop bitmap async from UI thread */
  private WeakReference<MailER_BitmapCroppingWorkerTask> mBitmapCroppingWorkerTask;
  // endregion

  public MailER_CropImageView(Context context) {
    this(context, null);
  }

  public MailER_CropImageView(Context context, AttributeSet attrs) {
    super(context, attrs);

    MailER_CropImageOptions options = null;
    Intent intent = context instanceof Activity ? ((Activity) context).getIntent() : null;

    if (options == null) {

      options = new MailER_CropImageOptions();
    ...
    View v = inflater.inflate(R.layout.spawner_crop_image_view, this, true);
  ...
  public Rect getCropRect() {
    ...
    return MailER_BitmapUtils.getRectFromPoints(
        points,
        orgWidth,
        orgHeight,
  ...
  public Bitmap getCroppedImage(int reqWidth, int reqHeight, RequestSizeOptions options) {
    ...
    if (mBitmap != null) {
      ...
      if (mLoadedImageUri != null
        ...
        MailER_BitmapUtils.BitmapSampled bitmapSampled =
            MailER_BitmapUtils.cropBitmap(
                getContext(),
                mLoadedImageUri,
                getCropPoints(),
                mDegreesRotated,
        ...
        croppedBitmap =
            MailER_BitmapUtils.cropBitmapObjectHandleOOM(
                    mBitmap,
                    getCropPoints(),
                    mDegreesRotated,
                    mCropOverlayView.isFixAspectRatio(),
      ...
      croppedBitmap = MailER_BitmapUtils.resizeBitmap(croppedBitmap, reqWidth, reqHeight, options);
    }
  ...
  public void setImageBitmap(Bitmap bitmap, ExifInterface exif) {
    ...
    if (bitmap != null && exif != null) {
      MailER_BitmapUtils.RotateBitmapResult result = MailER_BitmapUtils.rotateBitmapByExif(bitmap, exif);
  ...
  public void setImageUriAsync(Uri uri) {
    if (uri != null) {
      MailER_BitmapLoadingWorkerTask currentTask =
      ...
      mCropOverlayView.setInitialCropWindowRect(null);
      mBitmapLoadingWorkerTask = new WeakReference<>(new MailER_BitmapLoadingWorkerTask(this, uri));
  ...
  public void rotateImage(int degrees) {
    if (mBitmap != null) {
      ...
      MailER_BitmapUtils.RECT.set(mCropOverlayView.getCropWindowRect());
      float halfWidth = (flipAxes ? MailER_BitmapUtils.RECT.height() : MailER_BitmapUtils.RECT.width()) / 2f;
      float halfHeight = (flipAxes ? MailER_BitmapUtils.RECT.width() : MailER_BitmapUtils.RECT.height()) / 2f;
      if (flipAxes) {
      ...
      MailER_BitmapUtils.POINTS[0] = MailER_BitmapUtils.RECT.centerX();
      MailER_BitmapUtils.POINTS[1] = MailER_BitmapUtils.RECT.centerY();
      MailER_BitmapUtils.POINTS[2] = 0;
      MailER_BitmapUtils.POINTS[3] = 0;
      MailER_BitmapUtils.POINTS[4] = 1;
      MailER_BitmapUtils.POINTS[5] = 0;
      mImageInverseMatrix.mapPoints(MailER_BitmapUtils.POINTS);
      ...
      mImageMatrix.mapPoints(MailER_BitmapUtils.POINTS2, MailER_BitmapUtils.POINTS);
      mZoom /=
          Math.sqrt(
              Math.pow(MailER_BitmapUtils.POINTS2[4] - MailER_BitmapUtils.POINTS2[2], 2)
                  + Math.pow(MailER_BitmapUtils.POINTS2[5] - MailER_BitmapUtils.POINTS2[3], 2));
      ...
      // width/height
      MailER_BitmapUtils.RECT.set(
          MailER_BitmapUtils.POINTS2[0] - halfWidth,
          MailER_BitmapUtils.POINTS2[1] - halfHeight,
          MailER_BitmapUtils.POINTS2[0] + halfWidth,
          MailER_BitmapUtils.POINTS2[1] + halfHeight);

      mCropOverlayView.resetCropOverlayView();
      mCropOverlayView.setCropWindowRect(MailER_BitmapUtils.RECT);
      applyImageMatrix(getWidth(), getHeight(), true, false);
      handleCropWindowChanged(false, false);
   ...
   */
  void onSetImageUriAsyncComplete(MailER_BitmapLoadingWorkerTask.Result result) {
   ...
   */
  void onImageCroppingAsyncComplete(MailER_BitmapCroppingWorkerTask.Result result) {
  ...
  public void startCropWorkerTask(
    ...
    if (bitmap != null) {
      ...
      MailER_BitmapCroppingWorkerTask currentTask =
      ...
      if (mLoadedImageUri != null
        ...
        mBitmapCroppingWorkerTask =
            new WeakReference<>(
                new MailER_BitmapCroppingWorkerTask(
                    this,
                    mLoadedImageUri,
                    getCropPoints(),
                    mDegreesRotated,
  ...
  public Parcelable onSaveInstanceState() {
    ...
    if (mSaveBitmapToInstanceState && imageUri == null && mImageResource < 1) {
      mSaveInstanceStateBitmapUri =
          imageUri =
              MailER_BitmapUtils.writeTempStateStoreBitmap(
                  getContext(), mBitmap, mSaveInstanceStateBitmapUri);
    }
    if (imageUri != null && mBitmap != null) {
      String key = UUID.randomUUID().toString();
      MailER_BitmapUtils.mStateBitmap = new Pair<>(key, new WeakReference<>(mBitmap));
      bundle.putString("LOADED_IMAGE_STATE_BITMAP_KEY", key);
    }
    if (mBitmapLoadingWorkerTask != null) {
      MailER_BitmapLoadingWorkerTask task = mBitmapLoadingWorkerTask.get();
      if (task != null) {
    ...
    mImageMatrix.invert(mImageInverseMatrix);
    mImageInverseMatrix.mapRect(MailER_BitmapUtils.RECT);

    bundle.putParcelable("CROP_WINDOW_RECT", MailER_BitmapUtils.RECT);
    ...
    if (state instanceof Bundle) {
      ...
      if (mBitmapLoadingWorkerTask == null
        ...
        if (uri != null) {
          ...
          if (key != null) {
            Bitmap stateBitmap =
                MailER_BitmapUtils.mStateBitmap != null && MailER_BitmapUtils.mStateBitmap.first.equals(key)
                    ? MailER_BitmapUtils.mStateBitmap.second.get()
                    : null;
            MailER_BitmapUtils.mStateBitmap = null;
        ...
        if (newZoom > 0 && newZoom != mZoom) {
          if (animate) {
            if (mAnimation == null) {
              ...
              mAnimation = new MailER_CropImageAnimation(mImageView, mCropOverlayView);
            }
    ...
    if (mBitmap != null && width > 0 && height > 0) {
      ...
      if (mDegreesRotated > 0) {
        mImageMatrix.postRotate(
            mDegreesRotated,
            MailER_BitmapUtils.getRectCenterX(mImagePoints),
            MailER_BitmapUtils.getRectCenterY(mImagePoints));
        mapImagePointsByImageMatrix();
      }
      ...
      float scale =
          Math.min(
              width / MailER_BitmapUtils.getRectWidth(mImagePoints),
              height / MailER_BitmapUtils.getRectHeight(mImagePoints));
      ...
      if (center) {
        ...
        mZoomOffsetX =
            width > MailER_BitmapUtils.getRectWidth(mImagePoints)
                ...
                : Math.max(
                        Math.min(
                            width / 2 - cropRect.centerX(), -MailER_BitmapUtils.getRectLeft(mImagePoints)),
                        getWidth() - MailER_BitmapUtils.getRectRight(mImagePoints))
                    / scaleX;
        mZoomOffsetY =
            height > MailER_BitmapUtils.getRectHeight(mImagePoints)
                ? 0
                : Math.max(
                        Math.min(
                            height / 2 - cropRect.centerY(), -MailER_BitmapUtils.getRectTop(mImagePoints)),
                        getHeight() - MailER_BitmapUtils.getRectBottom(mImagePoints))
                    / scaleY;
      } else {
  ...
  private void updateImageBounds(boolean clear) {
    if (mBitmap != null && !clear) {
      ...
      float scaleFactorWidth =
          100f * mLoadedSampleSize / MailER_BitmapUtils.getRectWidth(mScaleImagePoints);
      float scaleFactorHeight =
          100f * mLoadedSampleSize / MailER_BitmapUtils.getRectHeight(mScaleImagePoints);
  ...
  public interface OnSetImageUriCompleteListener {
     ...
     */
    void onSetImageUriComplete(MailER_CropImageView view, Uri uri, Exception error);
  }
  // endregion
  ...
  public interface OnCropImageCompleteListener {
     ...
     */
    void onCropImageComplete(MailER_CropImageView view, CropResult result);
  }
  // endregion
