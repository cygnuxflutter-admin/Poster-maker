 ...
 */
public class MailER_CropOverlayView extends View {
     ...
     */
    private final MailER_CropWindowHandler mCropWindowHandler = new MailER_CropWindowHandler();
    /**
    ...
    private Paint mBorderCornerCornerPaint;
    /**
     ...
     */
    private MailER_CropWindowMoveHandler mMoveHandler;
    /**
     ...
     */
    private MailER_CropImageView.Guidelines mGuidelines;
    /**
     * The shape of the cropping area - rectangle/circular.
     */
    private MailER_CropImageView.CropShape mCropShape;
    /**
    ...
    public MailER_CropOverlayView(Context context) {
        this(context, null);
    }

    public MailER_CropOverlayView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    public void resetCropOverlayView() {
        if (initializedCropWindow) {
            setCropWindowRect(MailER_BitmapUtils.EMPTY_RECT_F);
            initCropWindow();
            invalidate();
        }
    }
     ...
     * The shape of the cropping area - rectangle/circular.
     */
    public MailER_CropImageView.CropShape getCropShape() {
        return mCropShape;
    }
     ...
     * The shape of the cropping area - rectangle/circular.
     */
    public void setCropShape(MailER_CropImageView.CropShape cropShape) {
        if (mCropShape != cropShape) {
            mCropShape = cropShape;
            if (Build.VERSION.SDK_INT <= 17) {
                if (mCropShape == MailER_CropImageView.CropShape.OVAL) {
     ...
     */
    public MailER_CropImageView.Guidelines getGuidelines() {
        return mGuidelines;
    }
     ...
     * application.
     */
    public void setGuidelines(MailER_CropImageView.Guidelines guidelines) {
    ...
    public void setInitialCropWindowRect(Rect rect) {
        mInitialCropWindowRect.set(rect != null ? rect : MailER_BitmapUtils.EMPTY_RECT);
        if (initializedCropWindow) {
            initCropWindow();
            invalidate();
     ...
     */
    public void setInitialAttributeValues(MailER_CropImageOptions options) {
    ...
    private void initCropWindow() {
        ...
        float leftLimit = Math.max(MailER_BitmapUtils.getRectLeft(mBoundsPoints), 0);
        float topLimit = Math.max(MailER_BitmapUtils.getRectTop(mBoundsPoints), 0);
        float rightLimit = Math.min(MailER_BitmapUtils.getRectRight(mBoundsPoints), getWidth());
        float bottomLimit = Math.min(MailER_BitmapUtils.getRectBottom(mBoundsPoints), getHeight());
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mCropWindowHandler.showGuidelines()) {
            ...
            if (mGuidelines == MailER_CropImageView.Guidelines.ON) {
                drawGuidelines(canvas);
            } else if (mGuidelines == MailER_CropImageView.Guidelines.ON_TOUCH && mMoveHandler != null) {
    ...
    private void drawBackground(Canvas canvas) {
        ...
        float left = Math.max(MailER_BitmapUtils.getRectLeft(mBoundsPoints), 0);
        float top = Math.max(MailER_BitmapUtils.getRectTop(mBoundsPoints), 0);
        float right = Math.min(MailER_BitmapUtils.getRectRight(mBoundsPoints), getWidth());
        float bottom = Math.min(MailER_BitmapUtils.getRectBottom(mBoundsPoints), getHeight());

        if (mCropShape == MailER_CropImageView.CropShape.RECTANGLE) {
            ...
            mPath.reset();
            if (Build.VERSION.SDK_INT <= 17 && mCropShape == MailER_CropImageView.CropShape.OVAL) {
    ...
    private void drawCorners(Canvas canvas) {
        if (mBorderCornerPaint != null) {
            ...
            float w =
                    cornerWidth / 2
                            + (mCropShape == MailER_CropImageView.CropShape.RECTANGLE ? mBorderCornerOffset : 0);
            ...
            mBorderCornerPaint.setColor(Color.WHITE);
            ...
            int GUIDELINES_ON_TOUCH = 1;
            float DEFAULT_CORNER_LENGTH_DP = 20.0f;
            float mCornerLength = TypedValue.applyDimension(GUIDELINES_ON_TOUCH, DEFAULT_CORNER_LENGTH_DP, displayMetrics);

            canvas.drawLine(left - cornerOffset, top - cornerExtension, left - cornerOffset, top + mCornerLength, mBorderCornerPaint);
            canvas.drawLine(left, top - cornerOffset, left + mCornerLength, top - cornerOffset, mBorderCornerPaint);
            canvas.drawLine(right + cornerOffset, top - cornerExtension, right + cornerOffset, top + mCornerLength, mBorderCornerPaint);
            canvas.drawLine(right, top - cornerOffset, right - mCornerLength, top - cornerOffset, mBorderCornerPaint);
            canvas.drawLine(left - cornerOffset, bottom + cornerExtension, left - cornerOffset, bottom - mCornerLength, mBorderCornerPaint);
            canvas.drawLine(left, bottom + cornerOffset, left + mCornerLength, bottom + cornerOffset, mBorderCornerPaint);
            canvas.drawLine(right + cornerOffset, bottom + cornerExtension, right + cornerOffset, bottom - mCornerLength, mBorderCornerPaint);
            canvas.drawLine(right, bottom + cornerOffset, right - mCornerLength, bottom + cornerOffset, mBorderCornerPaint);
        }
    }
    ...
    private boolean calculateBounds(RectF rect) {
        ...
        float left = MailER_BitmapUtils.getRectLeft(mBoundsPoints);
        float top = MailER_BitmapUtils.getRectTop(mBoundsPoints);
        float right = MailER_BitmapUtils.getRectRight(mBoundsPoints);
        float bottom = MailER_BitmapUtils.getRectBottom(mBoundsPoints);
