...
final class MailER_BitmapCroppingWorkerTask
    extends AsyncTask<Void, Void, MailER_BitmapCroppingWorkerTask.Result> {

  // region: Fields and Consts

  /** Use a WeakReference to ensure the ImageView can be garbage collected */
  private final WeakReference<MailER_CropImageView> mCropImageViewReference;
  ...
  private final MailER_CropImageView.RequestSizeOptions mReqSizeOptions;
  ...
  MailER_BitmapCroppingWorkerTask(
      MailER_CropImageView cropImageView,
      Bitmap bitmap,
      float[] cropPoints,
      int degreesRotated,
      ...
      MailER_CropImageView.RequestSizeOptions options,
      Uri saveUri,
  ...
  protected Result doInBackground(Void... params) {
    try {
      if (!isCancelled()) {
        ...
        Bitmap bitmap =
            MailER_BitmapUtils.resizeBitmap(bitmapSampled.bitmap, mReqWidth, mReqHeight, mReqSizeOptions);

        if (mSaveUri == null) {
          return new Result(bitmap, bitmapSampled.sampleSize);
        } else {
          MailER_BitmapUtils.writeBitmapToUri(
  ...
  protected void onPostExecute(Result result) {
    if (result != null) {
      ...
      if (!isCancelled()) {
        MailER_CropImageView cropImageView = mCropImageViewReference.get();
