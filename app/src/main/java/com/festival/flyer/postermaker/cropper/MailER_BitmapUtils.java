...
final class MailER_BitmapUtils {
  ...
  static Bitmap resizeBitmap(
          Bitmap bitmap, int reqWidth, int reqHeight, MailER_CropImageView.RequestSizeOptions options) {
    try {
      if (reqWidth > 0
          && reqHeight > 0
          && (options == MailER_CropImageView.RequestSizeOptions.RESIZE_FIT
              || options == MailER_CropImageView.RequestSizeOptions.RESIZE_INSIDE
              || options == MailER_CropImageView.RequestSizeOptions.RESIZE_EXACT)) {

        Bitmap resized = null;
        if (options == MailER_CropImageView.RequestSizeOptions.RESIZE_EXACT) {
          resized = Bitmap.createScaledBitmap(bitmap, reqWidth, reqHeight, false);
        } else {
          int width = bitmap.getWidth();
          int height = bitmap.getHeight();
          float scale = Math.max(width / (float) reqWidth, height / (float) reqHeight);
          if (scale > 1 || options == MailER_CropImageView.RequestSizeOptions.RESIZE_FIT) {
            resized =
