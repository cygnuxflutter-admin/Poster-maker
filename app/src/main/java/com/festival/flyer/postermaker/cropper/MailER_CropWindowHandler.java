...
final class MailER_CropWindowHandler {
   ...
   */
  public MailER_CropWindowMoveHandler getMoveHandler(
      float x, float y, float targetRadius, MailER_CropImageView.CropShape cropShape) {
    MailER_CropWindowMoveHandler.Type type =
        cropShape == MailER_CropImageView.CropShape.OVAL
            ? getOvalPressedMoveType(x, y)
            : getRectanglePressedMoveType(x, y, targetRadius);
    return type != null ? new MailER_CropWindowMoveHandler(type, this, x, y) : null;
  }
   ...
   * @return the Handle that was pressed; null if no Handle was pressed
   */
  private MailER_CropWindowMoveHandler.Type getRectanglePressedMoveType(
      float x, float y, float targetRadius) {
    MailER_CropWindowMoveHandler.Type moveType = null;

    // Note: corner-handles take precedence, then side-handles, then center.
    if (MailER_CropWindowHandler.isInCornerTargetZone(x, y, mEdges.left, mEdges.top, targetRadius)) {
      moveType = MailER_CropWindowMoveHandler.Type.TOP_LEFT;
    } else if (MailER_CropWindowHandler.isInCornerTargetZone(
        x, y, mEdges.right, mEdges.top, targetRadius)) {
      moveType = MailER_CropWindowMoveHandler.Type.TOP_RIGHT;
    } else if (MailER_CropWindowHandler.isInCornerTargetZone(
        x, y, mEdges.left, mEdges.bottom, targetRadius)) {
      moveType = MailER_CropWindowMoveHandler.Type.BOTTOM_LEFT;
    } else if (MailER_CropWindowHandler.isInCornerTargetZone(
        x, y, mEdges.right, mEdges.bottom, targetRadius)) {
      moveType = MailER_CropWindowMoveHandler.Type.BOTTOM_RIGHT;
    } else if (MailER_CropWindowHandler.isInCenterTargetZone(
            x, y, mEdges.left, mEdges.top, mEdges.right, mEdges.bottom)
        && focusCenter()) {
      moveType = MailER_CropWindowMoveHandler.Type.CENTER;
    } else if (MailER_CropWindowHandler.isInHorizontalTargetZone(
        x, y, mEdges.left, mEdges.right, mEdges.top, targetRadius)) {
      moveType = MailER_CropWindowMoveHandler.Type.TOP;
    } else if (MailER_CropWindowHandler.isInHorizontalTargetZone(
        x, y, mEdges.left, mEdges.right, mEdges.bottom, targetRadius)) {
      moveType = MailER_CropWindowMoveHandler.Type.BOTTOM;
    } else if (MailER_CropWindowHandler.isInVerticalTargetZone(
        x, y, mEdges.left, mEdges.top, mEdges.bottom, targetRadius)) {
      moveType = MailER_CropWindowMoveHandler.Type.LEFT;
    } else if (MailER_CropWindowHandler.isInVerticalTargetZone(
        x, y, mEdges.right, mEdges.top, mEdges.bottom, targetRadius)) {
      moveType = MailER_CropWindowMoveHandler.Type.RIGHT;
    } else if (MailER_CropWindowHandler.isInCenterTargetZone(
            x, y, mEdges.left, mEdges.top, mEdges.right, mEdges.bottom)
        && !focusCenter()) {
      moveType = MailER_CropWindowMoveHandler.Type.CENTER;
   ...
   * @return the Handle that was pressed; null if no Handle was pressed
   */
  private MailER_CropWindowMoveHandler.Type getOvalPressedMoveType(float x, float y) {
    ...
    */
    ...
    MailER_CropWindowMoveHandler.Type moveType;
    if (x < leftCenter) {
      if (y < topCenter) {
        moveType = MailER_CropWindowMoveHandler.Type.TOP_LEFT;
